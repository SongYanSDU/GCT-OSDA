...
from main_models import (build_feature_extractor_lboro,  build_doc_model, build_domain_discriminator, GradientReversalLayer)
from data_loader import Lboro_dataset_6, Lboro_dataset_5, Lboro_dataset_4
import os
from main_losses_lboro import total_loss, mmd_loss
...
data5_4, label5_4, data6_4, label6_4) = Lboro_dataset_4()
(data1_5, label1_5, data2_5, label2_5, data3_5, label3_5, data4_5, label4_5,
data5_5, label5_5, data6_5, label6_5) = Lboro_dataset_5()
(data1, label1, data2, label2, data3, label3, data4, label4, data5, label5,
data6, label6) = Lboro_dataset_6()

# 将 label4, label5, label6 中的标签 4 和 5 都映射为第 4 类
label4 = np.where(label4 >= num_classes, num_classes, label4)
label5 = np.where(label5 >= num_classes, num_classes, label5)
label6 = np.where(label6 >= num_classes, num_classes, label6)

# 组合源域数据
source_data_1 = np.concatenate((data1_4, data2_4, data3_4), axis=0)
source_labels_1 = np.concatenate((label1_4, label2_4, label3_4), axis=0)

# 定义训练和测试对
data_pairs = [(source_data_1, source_labels_1, data4, label4),
              (source_data_1, source_labels_1, data5, label5),
              (source_data_1, source_labels_1, data6, label6),
              (source_data_1, source_labels_1, data4_5, label4_5),
              (source_data_1, source_labels_1, data5_5, label5_5),
              (source_data_1, source_labels_1, data6_5, label6_5)]
...
for i, (tr_data, tr_label, tt_data, tt_label) in enumerate(data_pairs):
    ...
    # 初始化模型
    feature_extractor = build_feature_extractor_lboro()
    ...
    for epoch in range(300):
        _, _, _, pseudo_labels =  test_predict(combined_model, x_train_target, x_train_source, tt_label[:num], gaussian_model)
        ...
        # ======= 推断目标域并识别未知 =======
        _, _, acc_p, _ = test_predict(combined_model, tt_data_2, x_train_source, tt_label[num:], gaussian_model)
        if (epoch + 1) % 20 == 0:
            print("gaussian_loss = ", tf.reduce_mean(gaussian_loss).numpy())
            print(f"Epoch: {epoch}, Accuracy_p: {acc_p}")  # 打印分类报告

    '''feats_target, _, _, pseudo_labels = test_predict(combined_model, tt_data_2, x_train_source, tt_label[num:],
                                                     gaussian_model)
    source_features, _ = combined_model(x_train_source)
    final_means = gaussian_model.means.numpy()
    final_logvars = gaussian_model.logvars.numpy()
    # Create a directory to save results
    save_dir = "results_lboro_4"
    # Save results
    result_data = {
        "source_features": source_features,  # (N, 4096)
        "source_labels": train_label,  # (N,)
        "target_features": feats_target,  # (N, feat_dim)
        "target_labels": tt_label[num:],  # (N,)
    ...
    save_path = os.path.join(save_dir, f"experiment_{i + 1}_nosep.mat")
